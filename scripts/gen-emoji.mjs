...
// Keep only what can actually end up in an alias. Aliases must contain a word,
// so the right question is not "is this reservable on its own" — nothing is —
// but "does this still pass validation once combined with text". Prefixing a
// letter asks exactly that, and keeps the picker honest against whatever
// classifyAlias enforces later.
const valid = []
const rejected = []
for (const ch of items) {
  const r = classifyAlias('a' + ch)
  const oneGrapheme = Array.from(
    new Intl.Segmenter('en', { granularity: 'grapheme' }).segment(ch)
  ).length === 1
  if (r.ok && oneGrapheme) valid.push(ch)
  else rejected.push({ ch, why: r.ok ? 'multi-grapheme' : r.reason })
}
...
// Emits src/lib/emoji-data.ts directly — run with: npx tsx scripts/gen-emoji.mjs
// One step, no intermediate JSON to leave lying around or drift out of sync.
const ORDER = ['Смайлы', 'Люди', 'Природа', 'Еда', 'Места', 'Занятия', 'Вещи', 'Символы', 'Флаги', 'Прочее']

const header = `// Generated set of every renderable single-grapheme emoji, grouped for browsing.
// Regenerated by scripts/gen-emoji.mjs — do not hand-edit.
//
// Membership is decided by two Unicode properties together, Assigned AND Emoji,
// tested on the whole codepoint (ch[0] of a surrogate pair is only the high
// surrogate and matches nothing). That pair excludes the two kinds of junk that
// otherwise slip in: unassigned codepoints inside ranges reserved for future
// emoji, which draw as tofu boxes, and assigned-but-not-emoji pictographs such
// as the alchemical symbols block.
//
// Each entry is then checked as 'a' + emoji against classifyAlias. Aliases must
// contain a word, so asking whether the emoji is reservable alone would reject
// everything; what matters is that it still validates once combined with text.
`

const lines = [header, 'export const EMOJI_GROUPS: { name: string; emojis: string[] }[] = [']
for (const name of ORDER) {
  const list = groups[name]
  if (!list?.length) continue
  lines.push('  { name: ' + JSON.stringify(name) + ', emojis: ' + JSON.stringify(list) + ' },')
}
lines.push(']', '', 'export const TOTAL_EMOJI = EMOJI_GROUPS.reduce((n, g) => n + g.emojis.length, 0)', '')

writeFileSync(new URL('../src/lib/emoji-data.ts', import.meta.url), lines.join('\n'))
console.log('\nнаписано src/lib/emoji-data.ts')
