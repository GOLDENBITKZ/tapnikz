...
const SECTIONS = [
  ...
  },
  {
    ...
    items: [
      {
        q: 'Как добавить эмодзи в название бизнеса?',
        a: [
          'В кабинете → вкладка «Профиль» → поле «Название бизнеса / Имя».',
          'Нажмите на иконку смайлика справа в поле — откроется набор эмодзи по категориям (Популярные, Бизнес, Еда, Услуги, Символы).',
          'Выберите эмодзи — он добавится к названию.',
          'Под полем есть готовые варианты вроде «☕ Кофейня» — нажмите, чтобы подставить целиком, или скопируйте кнопкой копирования.',
          'Эмодзи в названии заметно выделяет вашу страницу — так оформлены все популярные профили сервиса.',
        ],
      },
      {
  ...
  {
    id: 'alias',
    title: '🚀 Эмодзи-ссылки (застолбить символ)',
    items: [
      {
        q: 'Что такое эмодзи-ссылка?',
        a: [
          'Это короткий адрес из слова и эмодзи, который ведёт куда вы укажете — например tapni.kz/bizde😅 открывает ваш Instagram или сайт.',
          'Обязательно должно быть слово: только эмодзи занять нельзя. Иначе первый, кто взял 🚀, забрал бы его навсегда, и всем остальным этот символ был бы недоступен.',
          'Со словом вариантов бесконечно много — shop🚀, cafe🚀 и bizde🚀 спокойно существуют одновременно.',
          'Удобно для визиток, вывесок и рекламы: такой адрес легко продиктовать и запомнить.',
        ],
      },
      {
        q: 'Какие адреса можно занять?',
        a: [
          'Слово плюс эмодзи: tapni.kz/bizde😅, tapni.kz/shop🚀, tapni.kz/cafe☕.',
          'Просто слово тоже подойдёт: tapni.kz/moy-magazin.',
          'Только эмодзи без слова — нельзя.',
          'Латиница и цифры. Кириллица недоступна: буквы разных алфавитов выглядят одинаково, и этим пользуются мошенники.',
          'Максимальная длина — 32 символа, как и у обычного адреса tapni.kz/ник.',
        ],
      },
      {
        q: 'Как застолбить символ?',
        a: [
          'Откройте кабинет → вкладка «Профиль» → блок «Застолбить эмодзи-ссылку».',
          'Введите символ вручную или нажмите на иконку смайлика и выберите из списка.',
          'Система сразу покажет: свободно ли и к какой категории относится символ.',
          'Укажите ссылку назначения (куда вести) и нажмите «Застолбить символ».',
          'Готовые адреса можно скопировать одним нажатием и вставить в Instagram или на визитку.',
        ],
      },
      {
        q: 'Можно ли поменять, куда ведёт мой символ?',
        a: [
          'Да. Сам символ остаётся за вами навсегда, а адрес назначения можно менять сколько угодно раз.',
          'В блоке «Ваши символы» нажмите «Изменить», укажите новую ссылку и сохраните.',
          'Изменение применяется мгновенно — перепечатывать визитки и QR-коды не нужно.',
        ],
      },
      {
        q: 'Почему мой символ не принимается?',
        a: [
          '«Занято» — символ уже застолбил другой пользователь. Попробуйте другой или добавьте второй эмодзи.',
          '«Системный адрес» — это служебное слово сервиса (например dashboard, pay, kaspi), его занять нельзя.',
          '«Только латиница и эмодзи» — кириллица недоступна: буквы разных алфавитов выглядят одинаково, и это используют мошенники для подделки ссылок.',
          '«Максимум 32 символа» — сократите ссылку.',
          '«Слишком длинная комбинация эмодзи» — составные эмодзи (например семья 👨‍👩‍👧‍👦) занимают много места; уберите часть.',
        ],
      },
      {
        q: 'Что будет с символом, если Premium закончится?',
        a: [
          'Ссылка продолжит работать — символ закреплён за вами.',
          'Чтобы менять адрес назначения, Premium должен быть активен.',
        ],
      },
    ],
  },
    ...
    items: [
      {
        ...
        a: [
          ...
          'Эмодзи-ссылка: застолбить короткий адрес вроде tapni.kz/🚀 (VIP, Double или Combo).',
          'Все типы контента: FAQ, видео, прайс-лист, таймер, баннер, форма заявок, Smart QR.',
